...
{
    public class MarketaStrategie: HerniStrategieTahniPrvniMoznouFigurkou
    {
		public MarketaStrategie(Hra hra) : base(hra)
		{
		}
		...
		public override Figurka? DejFigurkuKterouHrat(Hrac hrac, int hod)
		{
			foreach (var figurka in hrac.Figurky)
			{
				...
				if (cilovePolicko != null)
				{
					...
					else if (!cilovePolicko.ZjistiFigurkyHrace(hrac).Any() && !cilovePolicko.JeDomecek && cilovePolicko.ZjistiFigurkyProtihracu(hrac).Any())
					{
